...
async function getAccountId(email) {
  const url = `https://member-core-v2-be-svc.preprod-platform-cluster.tiket.com/tix-member-core/v2/account/account-id?by=EMAIL&memberType=ADMIN&value=${email}`;
  try {
...
async function getAccountDetails(accountId) {
  const url = `https://member-sidekick-go-be-svc.preprod-platform-cluster.tiket.com/tix-member-role-privilege/v1/account/detail?accountId=${accountId}`;
  try {
    const response = await fetch(url, { method: "GET", headers });
...
async function assignRolesToTargetAccount(targetAccountId, roleIds) {
  const url = "https://member-sidekick-go-be-svc.preprod-platform-cluster.tiket.com/tix-member-role-privilege/v1/account/role";
...
async function assignPrivilegesToTargetAccount(targetAccountId, privIds) {
  const url = "https://member-sidekick-go-be-svc.preprod-platform-cluster.tiket.com/tix-member-role-privilege/v1/account/privilege";
  const payload = JSON.stringify({
    accountId: targetAccountId,
    isBulk: true,
    privIds: privIds
