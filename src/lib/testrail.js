...
module.exports = {
  ...
  getCase: (caseId) => request('GET', `get_case/${caseId}`),
  getSection: (sectionId) => request('GET', `get_section/${sectionId}`),
  getCasesInSection: (projectId, suiteId, sectionId) =>
    getPaginated(
      `get_cases/${projectId}${suiteId ? `&suite_id=${suiteId}` : ''}&section_id=${sectionId}`,
      'cases'
    ),
