...
async function generateOtp(recipient) {
  ...
  const url =
    "https://sandbox.eph.bliblitiket.tools/gks-unm-go-be/api/v1/otp/generate";
...
async function verifyOtp(otpId, otpCode) {
  ...
  const url =
    "https://sandbox.eph.bliblitiket.tools/gks-unm-go-be/api/v1/otp/verify";
...
async function submitRegistration(
  ...
  const url =
    "https://sandbox.eph.bliblitiket.tools/gks-unm-go-be/api/v1/registration/submit";
...
async function fetchAccountId(email) {
  console.log(
    `Waiting 2 seconds before fetching account ID for email: ${email}`
  );
  await new Promise((resolve) => setTimeout(resolve, 2000));

  console.log(`Fetching account ID for email: ${email}`);
  const url = `https://member-core-v2-be-svc.preprod-platform-cluster.tiket.com/tix-member-core/v2/account/account-id?by=EMAIL&memberType=B2C&value=${encodeURIComponent(
    email
  )}`;
    ...
    .on("end", async () => {
      ...
      for (const user of users) {
        try {
          ...
          if (!passCode) {
            console.log(
              `Skipping user ${user.Email}, OTP verification failed.`
            );
            continue;
          }
      ...
      const csvOutput = stringify(users, {
        header: true,
        columns: [
          "Name",
          "Email",
